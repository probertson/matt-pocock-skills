...
**Fork note — the plugin marketplace.** This is a fork of `mattpocock/skills` that adds `.claude-plugin/marketplace.json`, a native Claude Code plugin marketplace so each skill can be installed on its own. `plugin.json` above is upstream's install-everything bundle — Matt maintains it, and we take it verbatim on every upstream sync. This fork's job is to keep `marketplace.json` covering the skills Matt lists in the **README Reference** — his curated, human-facing set — which is the surface this fork prepends to, so everything the README recommends is individually installable. The marketplace covers that Reference **plus its referral closure**: a skill Matt omits from the README but that a *covered* skill hard-routes to still ships, because a user who installs the covered skill would otherwise hit a dangling reference. Currently the only such skill is `implement` (a general-purpose PRD→implement→review orchestrator): the README leaves it out, but `ask-matt` presents `/implement` as the central build step, so it rides in as a standalone `mp-implement` that `mp-workflow` depends on. Everything else in `plugin.json` (upstream's fuller bundle) that is neither in the README nor referred to this way is deliberately excluded — the marketplace stays a subset of the bundle. `node sync/validate-marketplace.mjs` enforces that coverage (README Reference + justified referral closure, and each referral's referrer must still point at it) and *warns* about any `plugin.json` skill we're excluding, so each call stays conscious.
